...
// Google News RSS 503s from datacenter IPs (Google anti-bot block), so Bing
// News is tried first and Google News is used as a fallback.
const SOURCES = [
  {
    name: 'Bing News',
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(200).json({ success: false, count: 0, data: [], error: lastError });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return res.status(200).json({ success: false, count: 0, data: [], error: message });
  }
}
